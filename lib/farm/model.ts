...
export function emptyFarm(items: ContentItem[]): FarmState {
  return { entered: false, achievements: [], sources: [...new Set(items.map(i => i.favlist))], crops: [], reviewed: [], log: [], round: 1, gardenSince: null };
}
...
export function reduceFarm(farm: FarmState, action: FarmAction, items: ContentItem[]): FarmState {
  switch (action.type) {
    ...
    case "PLANT": {
      ...
      return { ...farm, crops: [...farm.crops, crop], gardenSince: farm.gardenSince ?? crop.plantedAt };
    }
    case "CARE": {
    ...
    case "NEXT_ROUND":
      return roundProgress(farm, items).complete ? { ...farm, round: farm.round + 1, reviewed: [] } : farm;
    case "ADVANCE":
