...
export function AchievementPanel({ farm, onClose }: { farm: FarmState; onClose: () => void }) {
  const varieties = ["小白菜", "樱桃萝卜", "牵牛花", "花椰菜", "葡萄", "豌豆苗"];
  const collected = new Set(farm.log.map(entry => (entry as any).assetFamily).filter(Boolean));
  const families = ["cabbage", "radish", "morningGlory", "cauliflower", "grape", "peaShoots"];
  return <Dialog title="每一次收获，都有新发现" eyebrow="作物图鉴" onClose={onClose}>
    <p className="muted">第一次收获一个品种，就会点亮它的图鉴。</p>
    <div className="achievement-gallery">{varieties.map((name, index) => <div key={name} className={"achievement-card " + (collected.has(families[index]) ? "earned" : "")}><Icon name={collected.has(families[index]) ? "trophy" : "leaf"}/><strong>{name}</strong><small>{collected.has(families[index]) ? "已收获 · 图鉴已点亮" : "等待第一次收获"}</small></div>)}</div>
  </Dialog>;
}
