...
import { roundProgress, stageOf } from "../../lib/farm/model";
import { AssetImage, FarmScene, Mascot, SceneBackdrop } from "./scene";
import { CropPanel, HelpPanel, LogPanel, MapPanel, SettingsPanel, SourcePanel } from "./panels";
import { Icon } from "./icon";
import { useFarm } from "./use-farm";

type Panel = "sources" | "import" | "log" | "help" | "map" | "settings" | { plot: number } | null;
export default function Game() {
  const game = useFarm(), [panel, setPanel] = useState<Panel>(null);
  const { farm, items, mode, now } = game;
  const progress = roundProgress(farm, items), mature = farm.crops.filter(c => stageOf(c, now) === "mature").length;
  ...
  const overlays = <>
    {(panel === "sources" || panel === "import") && <SourcePanel key={mode} game={game} onClose={close} importFirst={panel === "import"} />}
    {panel === "log" && <LogPanel farm={farm} items={items} mode={mode} now={now} onClose={close} onNewRound={() => { game.dispatch({ type: "NEXT_ROUND" }); game.setNotice("新一轮开始了，收获记录都为你保留。"); close(); }} />}
    {panel === "help" && <HelpPanel onClose={close} />}
    {panel === "map" && <MapPanel onClose={close} />}
  ...
  return <main className={"game-shell " + (!farm.entered ? "title-screen" : "")}>
    ...
    <header className="game-hud"><div className="brand"><span className="brand-mark"><Icon name="leaf" /></span><span>看山沃野<small>让收藏重新长出来</small></span></div>
      {farm.entered ? <><button className="hud-source" onClick={() => setPanel("sources")}><Icon name="folder" /><span><small>这轮回忆来自</small><strong>{farm.sources.length ? farm.sources[0] + (farm.sources.length > 1 ? " 等 " + farm.sources.length + " 个收藏夹" : "") : "还未选择收藏夹"}</strong></span><span className="source-edit">更换</span></button><div className="hud-right"><button className="hud-progress" onClick={() => setPanel("log")} aria-label={"本轮已收获 " + progress.count + " 篇，共 " + progress.total + " 篇，查看收获记录"}><span className="progress-ring" style={{ background: "conic-gradient(#42684a " + (progress.total ? progress.count / progress.total * 360 : 0) + "deg, #d8e0c9 0deg)" }}><Icon name="leaf" /></span><span><strong>{progress.count}<i> / {progress.total}</i></strong><small>本轮已收获</small></span></button><button className="icon-button hud-settings" onClick={() => setPanel("settings")} aria-label="农场设置"><Icon name="settings" /></button></div></> : <button className="quiet" onClick={() => setPanel("help")}><Icon name="help" />怎么玩</button>}
    </header>
    {farm.entered ? <>
      <FarmScene crops={farm.crops} now={now} event={game.event} onPlot={plot => { if (farm.crops.some(c => c.plot === plot)) setPanel({ plot }); else game.plant(plot); }} onMap={() => setPanel("map")} />
