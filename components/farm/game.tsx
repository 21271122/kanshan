...
export default function Game() {
  ...
  const chatSnapshot = makeFarmChatSnapshot(game.workspace, farm, !!game.auth?.authenticated, game.auth?.user?.name, mature);
  ...
  const overlays = <>
    ...
    {panel === "chat" && <ChatPanel snapshot={chatSnapshot} getSnapshot={game.getChatSnapshot} onClose={close} />}
  ...
  return <main className={"game-shell " + (!farm.entered ? "title-screen" : "")}>
    ...
    </> : <section className="welcome"><div className="welcome-copy"><p className="eyebrow">刘看山的收藏农场</p><h1>从前收藏的，<br/>今天又长出来了。</h1><p className="welcome-description">把忘在角落的好内容，种成一片小小沃野。<br/>看山替你照料，想起时，再来遇见。</p><div className="welcome-actions">{game.auth?.authenticated ? <button className="primary" onClick={() => game.start("personal")}>进入我的农场<Icon name="arrow" /></button> : <button className="primary" onClick={() => game.login()}>登录知乎，进入我的农场<Icon name="arrow" /></button>}<button className="secondary" onClick={() => game.start("demo")}>暂不登录，查看演示<Icon name="arrow" /></button></div></div><div className="welcome-mascot"><Mascot event={null} matureCount={0} onOpen={() => setPanel("chat")}/></div></section>}
